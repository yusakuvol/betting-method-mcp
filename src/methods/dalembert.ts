 ...
 *
 * The D'Alembert method is a more conservative progression system named after
 * the 18th-century French mathematician Jean le Rond d'Alembert.
 * - Start with a base unit bet amount
 * - On win: Decrease bet by 1 unit (minimum is base unit)
 * - On loss: Increase bet by 1 unit
 * - Session ends when reaching profit or max bet limit
 */
export class DAlembertMethod {
  ...
  recordResult(result: BetResult): void {
    ...
    if (result === "win") {
      ...
      // Decrease bet by 1 unit (minimum is base unit)
      const nextBet = Math.max(this.state.baseUnit, currentBetAmount - this.state.baseUnit);
      this.state.currentBet = nextBet;

      // Check if we've reached profit (optional session end condition)
      // For now, we keep the session active
    } else {
      // loss
      this.state.totalProfit -= currentBetAmount;
