...
describe("MartingaleMethod", () => {
  ...
  describe("statistics", () => {
    ...
    it("should return undefined when statistics not initialized", () => {
      const martingale2 = new MartingaleMethod();
      const stats = martingale2.getStatistics();
    ...
    it("should return deep copy of history arrays", () => {
      martingale.initSession(10);
      martingale.recordResult("win");
      const stats1 = martingale.getStatistics();
      const stats2 = martingale.getStatistics();
    ...
    it("should handle undefined history arrays", () => {
      ...
      (martingale as any).state.statistics = {
        // biome-ignore lint/suspicious/noExplicitAny: Testing private state access
        ...(martingale as any).state.statistics,
    ...
    it("should handle defined bankrollHistory", () => {
      ...
      expect(stats?.bankrollHistory).not.toBe(
        // biome-ignore lint/suspicious/noExplicitAny: Testing private state access
        (martingale as any).state.statistics.bankrollHistory,
      );
    });
    ...
    it("should initialize statistics if missing in recordResult", () => {
      martingale.initSession(10);
      // biome-ignore lint/suspicious/noExplicitAny: Testing private state access
      (martingale as any).state.statistics = undefined;
      martingale.recordResult("win");
      const stats = martingale.getStatistics();
      expect(stats).toBeDefined();
      expect(stats?.totalGames).toBe(1);
